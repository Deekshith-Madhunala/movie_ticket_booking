...
const AdminDashboard = () => {
    ...
    return (
        <Box sx={{ padding: 4, backgroundColor: '#f5f5f5', borderRadius: '8px' }}>
            ...
            {/* Loader: Display CircularProgress while loading */}
            {loading ? (
                <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '300px' }}>
                    <CircularProgress />
                </Box>
            ) : (
                // Cards and Tables go here once loading is complete
                <Grid container spacing={4} justifyContent="start" mt={1}>
                    ...
                    {/* More tables can go here for Theater, Booking, and User Details */}
                    <Grid item xs={12} md={6}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                            <Typography variant="h5" textAlign="start" sx={{ marginBottom: 2, fontWeight: 'bold' }}>
                                Theaters Details
